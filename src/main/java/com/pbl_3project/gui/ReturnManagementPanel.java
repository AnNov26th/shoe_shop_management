...
public class ReturnManagementPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        tableRequests.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                if (row >= 0) {
                    ...
                    lblReturnInfo.setText(
                            String.format("<html><b>Loại:</b> %s | <b>Lý do:</b> %s<br><b>Chi tiết:</b> %s</html>",
                                    type, reason, details));
        ...
        btnExchangeSelect.addActionListener(e -> {
            ...
            String info = JOptionPane.showInputDialog(this, "Nhập thông tin sản phẩm đổi (Tên, Size, Màu):",
                    "Chọn sản phẩm đổi", JOptionPane.QUESTION_MESSAGE);
            if (info != null && !info.trim().isEmpty()) {
                try {
                    if (orderBUS.updateExchangeInfo(orderId, info)) {
                        JOptionPane.showMessageDialog(this, "Đã lưu thông tin sản phẩm đổi!");
                        lblReturnInfo.setText(lblReturnInfo.getText().replace("</html>", "")
                                + " | <font color='blue'>SP Đổi: " + info + "</font></html>");
                    }
